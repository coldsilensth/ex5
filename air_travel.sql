...
SELECT "Trip" FROM "Trip" where town_from = 'Moscow';

SELECT * FROM passenger
WHERE name LIKE '%man';

SELECT "Trip" from "Trip" where plane = 'TU-134';

SELECT company_id from  "Trip" where town_to = 'Boeing';

SELECT plane from "Trip" where town_to = 'Moscow';

SELECT company_id from "Trip" where town_from = 'Vladivostok';

-- SELECT trip_id, count(passenger_id) as count from pass_in_trip;

SELECT DISTINCT town_to from "Trip"
    join pass_in_trip pit on "Trip".id = pit.trip_id join
    Passenger P on pit.passenger_id = P.id where name = 'Bruce Willis';

SELECT time_in from "Trip" join Pass_in_trip Pit on "Trip".id = Pit.trip_id
...
SELECT name, count(*) as flights_count
FROM passenger
GROUP BY name
ORDER BY flights_count DESC,name ASC;
